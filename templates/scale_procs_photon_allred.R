...
mydata <- read.csv("/Users/udayangawickramasinghe/Desktop/Summer2016/quals/Q_Andrew/paper/data_photon_performance/cutter/x_procs_y_time_photon_latency/excel/allred.csv", header = TRUE)
...
MYPLOT_PROPS['axis.range.y.limit', 'b']=195

#x axis
MYPLOT_PROPS['axis.range.x', 'active']=TRUE
MYPLOT_PROPS['axis.range.x.break', 'a']=2
MYPLOT_PROPS['axis.range.x.break', 'b']=14
MYPLOT_PROPS['axis.range.x.break', 'step']=1

dataset1 <- summarySE(mydata, measurevar="time", groupvars=c("mode","procs"), na.rm=FALSE,
                      conf.interval=.85)

dataset1
myplot2(dataset1, "procs", "time", "mode", "ci", "Allreduce Small Message (4byte) Latency on Photon, LibNBC (OFED) and MPI (cutter/224 cores)")

savelast_plot(name="allred_4_final")
